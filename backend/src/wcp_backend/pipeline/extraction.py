...
def resolve_classification(raw_classification: str) -> str:
    ...
    # Check if input is already canonical (case-insensitive)
    for canonical in set(IN_MEMORY_ALIASES.values()):
        if normalized == canonical.lower():
            return canonical
    
    # Return original if no match found (fuzzy matching is Phase 2)
